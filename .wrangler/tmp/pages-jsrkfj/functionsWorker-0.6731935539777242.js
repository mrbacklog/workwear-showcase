...
// ../src/lib/stable-url.ts
var EAN_RE = /^\d{13}$/;
var CV_HEX_RE = /^[0-9a-f]{4,}$/i;
var MODEL_ID_RE = /^[0-9a-zA-Z]{4,}$/;
function parseShowcaseTail(slugOrTail) {
...
__name(parseShowcaseTail, "parseShowcaseTail");
function shardOf(tail) {
  const raw = tail.length >= 2 ? tail.slice(-2) : tail;
  const cleaned = raw.toLowerCase().replace(/[^a-z0-9]/g, "_");
  return cleaned.length > 0 ? cleaned : "_";
}
__name(shardOf, "shardOf");
function buildRedirectTarget(origin, entry) {
...
__name(buildRedirectTarget, "buildRedirectTarget");
...
// ../.wrangler/tmp/pages-jsrkfj/functionsRoutes-0.3142272995040425.mjs
var routes = [
  {
